...
// LED pin definitions for overflow and underflow
#define OVERFLOW_LED_PIN 5
#define UNDERFLOW_LED_PIN 6
...
uint8_t overflow_flag, underflow_flag; // Flags for overflow and underflow

// Previous sng_en state for edge detection
uint8_t prev_sng_en = 0;

// Melody definitions (unchanged)
const int melody0[][2] = {
...
const int melody5[90][2] = {
    ...
    {0, 1000}, {0, 1000}, {0, 1000}, {0, 1000},
    {NOTE_D4, 1000}, {NOTE_D4, 1000}, {NOTE_D4, 500}, {0, 1000}
};
...
void setup() {
  ...
  pinMode(OVERFLOW_LED_PIN, OUTPUT); // Set overflow LED pin as output
  pinMode(UNDERFLOW_LED_PIN, OUTPUT); // Set underflow LED pin as output
}
...
// In the loop() function, replace the parsing section with this:
...
void loop() {
  if (new_data) {
    ...
    // Fixed bit extraction to match Verilog SPI packing:
    // Byte 3: {sng_en, overflow, underflow, 1'b0, pc_out[3:0]}
    sng_en = (data_buffer[3] >> 7) & 0x01;        // Bit 7: sng_en
    overflow_flag = (data_buffer[3] >> 6) & 0x01; // Bit 6: overflow 
    underflow_flag = (data_buffer[3] >> 5) & 0x01; // Bit 5: underflow
    ...
    Serial.print(operand, HEX);
    Serial.print(" OVF=");
    Serial.print(overflow_flag);
    Serial.print(" UNF=");
    Serial.println(underflow_flag);
    ...
    digitalWrite(OVERFLOW_LED_PIN, overflow_flag ? HIGH : LOW); // Control overflow LED
    digitalWrite(UNDERFLOW_LED_PIN, underflow_flag ? HIGH : LOW); // Control underflow LED
